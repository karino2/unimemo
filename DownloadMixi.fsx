...
let mixiPath = $"{unimemoRoot}/original/mixi"
let mixiGetPath = $"{mixiPath}/mixi_get.sh"
...
let listLD year =
    let di = DirectoryInfo  $"{unimemoRoot}/original/mixi/list_diary/"
    di.GetFiles()
...
let listLV () =
    let di = DirectoryInfo  $"{unimemoRoot}/original/mixi/list_voice/"
    di.GetFiles()
